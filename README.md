...
После урока вы сможете: **Измеряйте latency и throughput до оптимизации.**
...
- Профилирование event loop и CPU
- Кэширование и инвалидация
- Compression, ETag и reverse proxy
- Structured logs, metrics, traces и graceful shutdown
...
Разделяйте latency, throughput, error rate и saturation. Смотрите p50/p95/p99 на реалистичной нагрузке: среднее скрывает медленный хвост. Синхронный I/O и CPU-цикл блокируют все запросы процесса.

### Кэш и observability

Кэш требует ключа, TTL, лимита и стратегии invalidation. Structured logs связывайте request id, метрики используйте для trend/alert, tracing — для пути запроса через сервисы.

## Практика

1. Соберите минимальный пример по теме «6.7. Производительность, наблюдаемость и надёжность».
